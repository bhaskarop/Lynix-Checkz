...
async def api(client: Client, m: Message):
    ...
    if not url or not url_validator(url):
        return await m.reply("<b>Enter a valid url!</b>", quote=True)
    ...
    try:
        ...
        return await msg.edit(
            f"""<b>{message}!</b> Checked by - <a href='tg://user?id={m.from_user.id}'>
    ...
    await msg.edit(
        f"""<b>
Site - <code>{site}</code>
Price - <code>{total[:2]}$</code>

Status - <code>{status}</code>
Result - <code>{response_site}</code>

Gateway - Api
Time - <code>{time}'s</code>
