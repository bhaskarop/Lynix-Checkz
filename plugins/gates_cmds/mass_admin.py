...
msg_error = "Gateway Mass\nFormat - <code>/msa gate cc|month|year|cvc</code>"
...
async def mass_admin(client: Client, m: Message):
    ...
    msg_bot = await m.reply("Please Wait...", quote=True)
    text_ccs = ""
    errors = 0
    approveds = 0
    deads = 0
    for i in range(0, len(ccs_findeds)):
        ...
        if "approved" in status.lower():
            ...
            errors += 1
        text_ccs += f"""<b>CC - <code>{ccf}</code>
Status - <code>{status}</code>
Result - <code>{msg}</code></b>\n
"""
...
{text_ccs} Time - <code>{final:0.4}'s</code>
Credits - <code>{user_info.get('CREDITS')}</code>
Total Cards - <code>{len(ccs_findeds)}</code>
