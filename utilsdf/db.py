...
class Database:
    ...
    def remove_credits(self, user_id: int, credits: int) -> None:
        ...
        if result is None:
            return
        credits_user = result[0]
        new_credits = max(credits_user - credits, 0)
        self.cursor.execute(
    ...
    def is_authorized(self, user_id: int, chat_id: int = None) -> bool:
        user_id = int(user_id)
        if str(user_id) == self.ID_OWNER:
            return True
        if self.is_seller_or_admin(user_id):
            return True
        if self.is_premium(user_id):
            return True
        if self.user_has_credits(user_id):
            return True
        return False
