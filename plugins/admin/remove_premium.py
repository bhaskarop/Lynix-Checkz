    ...
    with Database() as db:
        ...
        if not id or (id[0] != "-" and not id.isdigit()) or not id[1:].isdigit():
            return await m.reply(
                "<b>Enter a valid ID to reset!</b>", quote=True
            )
        ...
        if result is None:
            return await m.reply(
                "<b>The ID is not found in the database!</b>", quote=True
            )
        await m.reply(f"<b>The ID <code>{id}</code> has been reset!</b>", quote=True)
