...
async def restart_bot(client: Client, m: Message):
    with Database() as db:
        if not db.is_admin(m.from_user.id):
            return
    msg = await m.reply("<b> Restarting bot...</b>", quote=True)
    await sleep(1)
...
async def system_stats(client: Client, m: Message):
    ...
    text = f"""<b>📊 System Status

⚙️ OS - <code>{info['os']}</code>
⏱ Uptime - <code>{info['uptime']}</code>

🖥 CPU - <code>{info['cpu']}</code>
💾 RAM - <code>{info['ram_used']}/{info['ram_total']} ({info['ram_percent']})</code>
💿 Disk - <code>{info['disk_used']}/{info['disk_total']} ({info['disk_percent']})</code>
</b>"""
    await msg.edit(text)
...
async def speedtest(client: Client, m: Message):
    ...
    try:
        ...
        await msg.edit(f"<b>🚀 Speedtest Result\n\n<code>{output}</code></b>")
    except asyncio.TimeoutError:
