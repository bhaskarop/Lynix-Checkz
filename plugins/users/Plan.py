...
Garry = 1718738592
Fucker_504 = 1718738592 # Co-Founder ID
...
async def plan(client: Client, m: Message):
 ...
 rol = info_user["RANK"] if str(user_id) != Database.ID_OWNER else "Owner"
 rol = rol.capitalize()
 if user_id == Fucker_504:
 rol = "Co-Founder"
 rol = f"\nRol - <code>{rol}</code>" if rol.lower() != "user" else ""
 nick = info_user["NICK"]
 ...
 expiraton_premium = (
 f"\nExp - <code>{days}d-{hours}h-{minutes}m-{seconds}s</code>"
 )
...
Plan - <code>{membership}</code>
Spam - <code>{antispam}</code> {rol} {expiraton_premium}
Credits - <code>{credits}</code>
Nick - <code>{nick}</code>
Reg - <code>{registered}</code>
"""
